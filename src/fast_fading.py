#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""This module computes the fast fading model of 3gpp scenarios.
...
class SSPs:
  ...
  like properties.
  """  
  def __init__(self,n_scatters=0):
    """
    ...
    """ The number of the scatters in this channel model. """ 
    self.reduced_n_scatters = n_scatters 
    """ The number of the scatters after filter the scatters with low power. """ 
    self.number_sps = 15
...
class FastFading3gpp:
  """This class implments the 3gpp fast fading model
  """  
  def __init__(self,scenario,MS_antenna,BS_antenna,scatters_move=False,move_probability =0,v_min_scatters=0,v_max_scatters=0):
    """
    The constructor method of the  FastFading3gpp Class.
    ...
    """ The scenario for the fast fading model.""" 
  ...
  def compute_ch_matrix(self,MS_pos,MS_vel,t=0,mode=0):
    """ 
    ...
    if mode == 0:
        self.__compute_ssps(MS_pos,True)
    else:
        self.__set_correlated_ssps(MS_pos)
  ...
  def __set_correlated_ssps(self,pos):
    ...
    i_x1,i_x2,i_y1,i_y2 = self.__find_point_meshgrid(pos,self.scenario.X,self.scenario.Y)
    ...
    points = []
    ssps_grid = []
    if los_p00 == self.__los: 
        if self.__ssp_grid[i_x1][i_y1]==None:
            self.__ssp_grid[i_x1][i_y1] = self.__compute_ssps(p00,False)
        points.append(p00)
        ssps_grid.append((self.__ssp_grid[i_x1][i_y1]))
        
    if los_p01 == self.__los:
        if self.__ssp_grid[i_x1][i_y2]==None:
            self.__ssp_grid[i_x1][i_y2] = self.__compute_ssps(p01,False)
        points.append(p01)
        ssps_grid.append((self.__ssp_grid[i_x1][i_y2]))
    if los_p10 == self.__los:
        if self.__ssp_grid[i_x2][i_y1]==None:
            self.__ssp_grid[i_x2][i_y1] = self.__compute_ssps(p10,False)
        points.append(p10)
        ssps_grid.append((self.__ssp_grid[i_x2][i_y1]))
    if los_p11 == self.__los:
        if self.__ssp_grid[i_x2][i_y2]==None:
            self.__ssp_grid[i_x2][i_y2] = self.__compute_ssps(p11,False)
        points.append(p11)
        ssps_grid.append((self.__ssp_grid[i_x2][i_y2]))
    
    if len(ssps_grid) > 0:
        ret_ssp = SSPs(ssps_grid[0].n_scatters)
        for i in range(ssps_grid[0].number_sps):
            values = []
            for ssp in ssps_grid:
                values.append(ssp.ssp_array[i])    
            ret_ssp.ssp_array[i] = self.__inverse_distance_sspgrid_interpol(pos, np.array(points),np.array(values))
        self.__filter_scatters(ret_ssp)
        self.__set_ssps(ret_ssp)
    else:
        print("Error: no points to interpolate")
        return None        


  def __inverse_distance_sspgrid_interpol(self,point, XY,values, p = 2):
      """
  ...
  def __compute_ssps(self,pos,filter_scatters):
        """
        ...
        ssp = SSPs(self.scenario.n_scatters) 
        self.reduced_n_scatters = self.scenario.n_scatters
        ...
        ssp.P,ssp.P_LOS = self.__gen_clusters_powers(filter_scatters)
        self.__P = ssp.P
  ...
  def __find_point_meshgrid(self,pos,X,Y):
  ...
  def update(self,mspos,msvel,t0,time,d2d,d3d):
